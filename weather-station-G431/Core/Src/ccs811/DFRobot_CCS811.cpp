 ...
 * @retval None.
 */
void Init_I2C_CCS811(I2C_HandleTypeDef i2c1) {
...
//	 */
//	if (HAL_I2CEx_ConfigAnalogFilter(&hi2cxc, I2C_ANALOGFILTER_ENABLE)
//			!= HAL_OK) {
//		while (1)
//			;
//	}
...
void readAlgorithmResults() {
	...
	uint8_t data_rq[4];
	uint8_t status = HAL_I2C_Mem_Read(&hi2cxc, CCS811_ADDRD,
			(uint8_t) CSS811_ALG_RESULT_DATA, I2C_MEMADD_SIZE_8BIT, data_rq, 4,
			100);
...
void configureCCS811() {
	...
	// TODO
	uint8_t hwID = readRegister(CSS811_HW_ID); //Hardware ID should be 0x81
	if (hwID != 0x81) {
	...
	HAL_I2C_Master_Transmit(&hi2cxc, CCS811_ADDRD, lodata, 1, 100); // 0xB6
...
FlagStatus checkForError() {
	...
	return static_cast<FlagStatus>(errvalue & 1 << 0);
}

//Checks to see if DATA_READ flag is set in the status register
FlagStatus dataAvailable() {   // dtvalue = readRegister(CSS811_ERROR_ID);
//HAL_Delay(00);
	dtvalue = readRegister(CSS811_STATUS);
	return static_cast<FlagStatus>(dtvalue & 1 << 3);
}

//Checks to see if APP_VALID flag is set in the status register
FlagStatus appValid() {
	appvalue = readRegister(CSS811_STATUS);
	return static_cast<FlagStatus>(appvalue & (1 << 4));
}
...
void setEnvironmentalData(float relativeHumidity, float temperature) {
	...
	env[3] = envData[2];
	env[5] = envData[3];
	HAL_I2C_Mem_Write(&hi2cxc, CCS811_ADDWR, (uint8_t) CSS811_ENV_DATA,
...
void read_Mul_Register(uint8_t addr, uint8_t *val, uint8_t size) {
	HAL_I2C_Mem_Read(&hi2cxc, CCS811_ADDRD, (uint8_t) addr,
			I2C_MEMADD_SIZE_8BIT, val, size, 100);
...
void softRest() {
	...
	HAL_I2C_Mem_Write(&hi2cxc, CCS811_ADDWR, CSS811_SW_RESET,
			I2C_MEMADD_SIZE_8BIT, rstCMD, 5, 300);
	while (HAL_I2C_GetState(&hi2cxc) != HAL_I2C_STATE_READY)
		;
...
uint32_t get_Sensor_Resistance() {
	...
	HAL_I2C_Mem_Read(&hi2cxc, CCS811_ADDRD, CSS811_RAW_DATA,
			I2C_MEMADD_SIZE_8BIT, sensor_Resistance_raw, 2, 100);
...
unsigned int getBaseline() {
	...
	status = HAL_I2C_Mem_Read(&hi2cxc, CCS811_ADDRD, (uint8_t) CSS811_BASELINE,
			1, ada, 2, 100);
	while (HAL_I2C_GetState(&hi2cxc) != HAL_I2C_STATE_READY) {
	}
	while (HAL_I2C_IsDeviceReady(&hi2cxc, CCS811_ADDRD, 10, 300) == HAL_TIMEOUT)
...
void writeRegister(uint8_t addr, uint8_t val) {
	...
	HAL_I2C_Mem_Write(&hi2cxc, CCS811_ADDWR, (uint8_t) addr,
